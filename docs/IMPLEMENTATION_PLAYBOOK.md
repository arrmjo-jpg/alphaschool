...
16. **Test environment isolation confirmed before the phase's test output is trusted** — before any major implementation phase, verify the test suite is actually isolated from the development database, not assumed isolated because `phpunit.xml` says so. `backend/tests/TestCase.php` makes this self-enforcing (it throws immediately if a test run ever resolves to a non-`sqlite`/`:memory:` connection, before `RefreshDatabase` can act on it), so in practice this item is satisfied by the guard being present and the suite running green — but if `phpunit.xml` or `TestCase.php` are ever touched, re-verify by the same live-reproduction method the incident itself used: seed the dev database, run one test, confirm the dev database's row count is unaffected. See `docs/developer/rca-2026-07-21-test-database-wipe.md` for why this was added — `php artisan test` silently wiped the real dev database on every run for a full day, undetected, because the isolation this item now requires didn't actually hold.
...
**Correction (2026-07-21, superseded by RCA — not a retraction of the verification discipline, a correction of its conclusion):** the diagnosis above was reached from real, honestly-reported evidence at the time — a genuine, reproducible `SQLSTATE[01000]` failure was observed, and the schema incompatibility it names (an `unsignedBigInteger` morph column receiving a ULID string) is real — but the conclusion that this was the *cause*, pre-existing and unrelated to Docker, was wrong. The actual cause was that `php artisan test` was silently connecting to the real MariaDB dev database instead of the isolated in-memory SQLite `phpunit.xml` declares (a `phpunit.xml`/Docker `env_file` precedence bug, unrelated to `Media`'s schema), which is why the failure only appeared once tests started running inside the `app` container: MariaDB enforces strict-mode column truncation, SQLite does not. With that bug fixed, the full suite passes 367/367 — this "12 failures" schema incompatibility does not manifest against the test suite's intended isolated database and was never something Media's schema needed to change for. Full root-cause analysis, live reproduction, and the fix: `docs/developer/rca-2026-07-21-test-database-wipe.md`.
...
**Provider Registry — §27 design review + Phase F-A COMPLETE (2026-07-20)**, per `docs/ADMIN_DESIGN_SYSTEM.md` §27, the second Administration child, sequenced immediately after Configuration Platform and before any new business-domain workspace by explicit instruction ("complete the Administration platform first"). A dedicated design review against the real Phase 2 backend (`ProviderManager`/`ProviderCredentialVault`/`HealthCheckRunner`) found three genuine differences from Configuration Platform's own design, not assumed to be a copy of it: credential values are never exposed in either direction (a write-only field renderer, not a reused one); no per-slot view permission exists in the backend to check (resolved as general Administration access, no per-slot gate); and a provider slot is one atomic all-or-nothing credential set, not a category with independently browsable fields (Overview Grid → direct form, no rail — `Configuration Platform = Overview Grid → Category → Settings Form`, `Provider Registry = Overview Grid → Provider Form`).

A pre-freeze review round incorporated four further refinements before §27 was considered frozen, each a real architectural decision, not wording: explicit backend-declared field types (`text`/`password`/`secret`) replacing a name-based heuristic that would only ever grow as new field names appeared; a binding Edit→Test→Save rule so Test Connection never persists an unsaved credential, requiring a new backend contract (`TestsCredentials`, a `HealthCheckable` sibling) scoped into Phase F-B; a client-only `Checking…` transient status explicitly distinguished from `HealthCheckRunner`'s four real states; and a rule against the UI ever branching on capability contracts or vendor identity, mirroring `ProviderManager`'s own backend discipline verbatim.

Phase F-A shipped the frontend infrastructure against a temporary fixture, mirroring Phase E-A's exact discipline. `admin/src/platform/administration/overview-grid.tsx` is the Overview Grid Pattern promoted from its original Configuration-Platform-only home to a shared component — its second real consumer, confirming §26.16's own claim it would generalize; Configuration Platform's usage was updated to the same import with zero behavior change, its old workspace-scoped copy deleted. `admin/src/platform/administration/provider-registry-provider.ts` is the new data contract; `admin/src/workspaces/administration-provider-registry/` holds the real page templates (`provider-registry-page.tsx`, `provider-credential-field.tsx`, `provider-credential-form.tsx`, `register.ts`, locales) — `register.ts` registers the workspace shell only, deliberately not wiring a real provider yet, exactly mirroring Configuration Platform's own E-A/E-B split.

Verified live against a five-slot fixture (all five statuses exercised, including catching the transient `checking`→`ready` transition mid-flight, not just its resting states), a `canEdit: false` slot proving the view-only path renders correctly with no Test/Save controls, and a real Test→Save round trip proving Test genuinely never persists — the pending form state and Save button survived a successful Test unchanged, only clearing after the actual Save. RTL/Arabic, dark mode, and mobile all confirmed. The fixture wiring (`App.tsx`, `use-visible-workspaces.ts`) was fully reverted before commit — `use-visible-workspaces.ts` came back byte-identical to its prior committed state, confirmed via `git diff` showing no trace of it at all. `tsc -b` clean, `oxlint` clean (same pre-existing warnings, none new). Phase F-B (the real backend adapter, plus the two new backend contracts this review specified) remains, its own separately-scoped task.

**Provider Registry — Phase F-B COMPLETE (2026-07-21)**, per `docs/ADMIN_DESIGN_SYSTEM.md` §27.13/§27.14, implemented incrementally per explicit instruction rather than as one change: backend contracts first, then Provider Registry/provider declarations, then `ProviderManager`/`ProviderCredentialVault` adaptation, then the real frontend adapter replacing the F-A fixture, then full-flow verification, then documentation. Both backend contracts §27.13 specified are live — `ProviderCredentialFieldDefinition` (`{name, type}`) replacing the plain `string[]` shape across `ProviderSlotDefinition`, `ProviderRegistry::sync()`, `ProviderCredentialVault::assertCredentialShape()`, and all four existing Providers; and `TestsCredentials` (`testCredentials(array $credentials): bool`), a `HealthCheckable` sibling implemented by all four Providers, satisfying the Edit→Test→Save rule without touching the Vault. A new `ProviderRegistryController` exposes the four-route adapter API, and `@alphaschool/contracts` gained a `providers` feature folder mirroring `settings`'s own shape.

One real architectural gap surfaced mid-implementation and was stopped on and resolved by explicit product decision before continuing, per this phase's own "surface before implementing" instruction: §27.6 as originally frozen assumed workspace visibility could reuse "general Administration access," but no such permission — nor any per-slot view permission — actually existed anywhere in `WorkspaceAccessResolver` to gate on. Resolved as a dedicated `administration.providers.view` permission (new `administration` permission group), independent of and never inferred from the union of per-slot edit permissions, mirroring the existing `is_super_admin` asymmetry (view bypasses, edit never does).

`register.ts` now wires `realProviderRegistryDataProvider` permanently, mirroring Phase E-A→E-B's own split exactly; the F-A fixture files were deleted, not archived. Verified: 11 new Pest feature tests (`ProviderRegistryControllerTest.php`) covering the full slots/detail/test/write/permission/version-conflict matrix over real HTTP, including a test proving Test Connection never persists by database state (fetch → test → fetch again, assert unchanged → save → fetch again, assert changed), not by UI sequencing alone. Full backend suite: 354 passing (12 pre-existing, separately-tracked, unrelated failures unchanged — **superseded, see the correction note below**). Live browser verification against the real Docker backend: all four real slots list with real status/module data; the R2 slot's detail view renders its four real backend-declared field types correctly (`key`/`secret` masked, `region`/`endpoint` plain) and correctly disables all four for a view-only user; RTL/Arabic, dark mode, and a clean network/console log all confirmed live. Edit/Test/Save/permission/version-conflict mechanics for a non-super-admin, edit-permitted user rely on the 11 Pest tests rather than live browser verification, for the same reason already documented in Phase E-B: Spatie Permission Teams' `setPermissionsTeamId()` only resolves within one PHP process, never across real HTTP requests.

A notable operational finding, unrelated to this phase's design or code: during live-verification setup, the database was found completely empty (every table, intact schema) with no root cause identified after ruling out container recreation, config-caching, and a wrong-database mixup; state was restored via `db:seed` and the existing sync commands. Recorded here for visibility, not yet separately investigated.

**Correction (2026-07-21):** both paragraphs above were accurate reports of what was known at the time, not errors — but the root cause has since been found, reproduced live, and fixed, and it retroactively changes the second sentence of the first paragraph. The empty-database incident and the "12 pre-existing failures" were the same root cause, not two separate findings: `php artisan test` was silently connecting to the real MariaDB dev database instead of the isolated in-memory SQLite `phpunit.xml` declares, so every test run dropped and recreated every table (`RefreshDatabase`'s `migrate:fresh`) in the real dev database, and the MariaDB-specific strict-mode column enforcement that produced those 12 failures doesn't apply once tests correctly run against SQLite. With the fix in place the full suite passes 367/367. Full RCA, live reproduction, and fix: `docs/developer/rca-2026-07-21-test-database-wipe.md`.
...
**Status: COMPLETE (2026-07-27).** Delivered exactly as scoped: `AcademicYear`/`GradeLevel`/`BranchGradeLevel` migrations and models, `AcademicCatalogService` as the public contract Admissions (Sprint 4.2) consumes exclusively, `ClosedAcademicYearGuard` enforcing the closed-year rejection as a real, tested guard. An independent read-only review (not the implementing agent's own report) caught one real deviation before certification: `academic_years.name_en` was missing the `UNIQUE` constraint the approved spec had explicitly named — fixed, plus the corresponding factory hardened against the same latent collision risk. Full verification (tests/Pint/PHPStan/migration/rollback) re-run and re-confirmed independently after the fix, not just re-asserted by the original report.
...
**Status: COMPLETE (2026-07-27).** Delivered as scoped, with one implementation-shape clarification made explicit in code: `StepUpAuthentication`'s real signature requires an already-authenticated `User`, so "submit application" necessarily assumes the guardian is already logged in (Identity/People's own existing concern, not built here); duplicate-detection's own named exercise point ("a returning guardian's new application must correctly find their existing Person/Guardian record") was accordingly read as applying to the *applying child*, not the guardian, whose identity is already exact via authentication. An independent read-only review — not the implementing agent's own report — found three real issues the original report missed, all confirmed by direct code inspection and fixed before certification:

1. **Transaction atomicity.** `GuardianVerificationService::resolveGuardian()` (which can write `Guardian`/`PersonIdentityDocument` rows for a first-time guardian) ran *before* `DB::transaction()` opened, so a failure later in the flow (e.g. Grade-Level-not-offered) left those writes permanently committed with no `Applicant` ever created. Fixed by moving the transaction boundary to wrap everything from `resolveGuardian()` through `Applicant::create()`.
2. **Missing validation.** `Applicant::reject()` accepted any `reason_codes.id` satisfying the foreign key, with no check it belonged to the `application_rejection` context. Fixed with an explicit context check, raising a new `InvalidRejectionReasonException` otherwise.
3. **N+1 query.** `GuardianVerificationService::toSignals()` lazy-loaded `identityDocuments` once per duplicate-detection candidate instead of eager-loading. Fixed with `->with('identityDocuments')` on the candidates query.

None of the three were caught by the original test suite — see "Lessons Learned" below for why, and what now changes going forward.

**Lessons Learned (binding for every future sprint in this Phase, not just this one):**

- **Review every new or moved `DB::transaction()` boundary explicitly**, not just whether the code inside it is individually correct. A transaction that opens *after* the first write it should be protecting is a silent, easy-to-miss defect — the code runs, the happy path passes, and only a test that deliberately fails *after* the early write ever catches it.
- **Any foreign key into a shared, multi-context table (`reason_codes`, and any future table with the same shape) needs an explicit context check at the point of use**, not just the database-level existence guarantee the FK itself provides. A valid row from the wrong context is still a valid row as far as the FK is concerned.
- **Every bug found during review gets a regression test that fails without the fix and passes with it** — not just a code change. All three fixes above shipped with one.
- **N+1 queries are now a standing item in every future code review in this Phase**, not just something caught incidentally. `Model::preventLazyLoading()` is not enabled anywhere in this codebase, so this class of bug degrades silently in production rather than failing loudly in dev/test — reviewers cannot rely on the test suite surfacing it on its own.
...
**Status: COMPLETE (2026-07-27).** Delivered as scoped: `ConvertApplicantToStudentAction`, `Billable`/`ApplicantFeeBillable`, `Enrollment`/`SuspensionRecord` as a complete aggregate (built one sprint ahead of the original 4.4 placement, per an explicit architectural revision made before implementation began — Sprint 4.4 now extends Enrollment *behavior* only, never its schema), `StudentEnrolled`/`StudentReactivated`/`ApplicantConverted` events, and full test coverage including a genuine dual-connection MariaDB concurrency proof for the double-conversion guard. An independent read-only repository review — run by mechanically executing `deptrac analyse`, not by trusting the implementation report — found one critical finding the implementation's own verification pass missed entirely, plus one medium-severity inconsistency introduced within the same sprint. Both fixed, with regression tests, before certification:

1. **Module boundary violation (critical).** The original approved specification placed `ConvertApplicantToStudentAction` and its three exceptions (`ApplicantNotAcceptedException`, `ApplicantAlreadyConvertedException`, `PaymentNotConfirmedException`) in `App\Modules\People` — reasoning about which module's *tables* get written, not which module's *code* does the importing. `deptrac analyse` reported 11 violations: `People` is `Foundation`, `Admissions`/`Academic` are `Domain`, and Foundation must never depend on Domain. Resolved via ADR-0026: the Action and its three exceptions moved to `App\Modules\Admissions`, where the dependency direction (Admissions → People, for `Student`/`Enrollment`) is the permitted one. `Student`/`Enrollment` themselves stay owned by People, unchanged — only the calling code's location moved. A related, narrower violation surfaced in the same review — `Enrollment::academicYear()`/`gradeLevel()` declared direct relations to Academic's Domain models — was fixed by removing both (confirmed unused anywhere in the codebase first); the underlying `academic_year_id`/`grade_level_id` FK columns are untouched.
2. **Event-dispatch timing inconsistency (medium).** `Applicant::convert()` dispatched `ApplicantConverted` synchronously while being called from inside `ConvertApplicantToStudentAction`'s open transaction — unlike its sibling event `StudentEnrolled`, added in the same sprint, which correctly deferred via `DB::afterCommit()`. Fixed at `convert()`'s own declaration point, matching `Student::reactivate()`'s existing pattern.

**Lessons Learned (binding for every future sprint):**

- **`deptrac` must actually be run, every sprint, not just documented as a Quality Gate.** It was already listed in this Playbook's own "Quality gates" checklist (`deptrac — zero module-boundary violations`) before Sprint 4.3, but was not run during this sprint's own verification pass (migrations, tests, Pint, PHPStan, rollback) — the omission, not a tooling gap, is what let a critical, mechanically-detectable violation reach independent review instead of being caught during implementation. `vendor/bin/deptrac analyse --no-cache` is now a mandatory step in every sprint's verification sequence, with no exception.
- **A module placement decision made on data-ownership grounds ("who writes this table") is not the same question as an import-direction decision ("who is allowed to depend on whom").** Both must be checked independently — the Sprint 4.3 specification's ownership reasoning was sound and remains sound (Admissions still never writes Student/Enrollment tables directly, through its own models); the layering violation was a separate, unrelated failure to check the calling code's own module classification against `deptrac.yaml` before implementation began.
- **When one event in a batch of same-sprint additions gets an `afterCommit()` fix and a sibling event doesn't, that's a signal to check every event added in the same change, not just the one a regression was written for.** `StudentEnrolled` and `ApplicantConverted` were added in the same sprint, dispatched from the same transaction, and only one initially got the fix.
- **`deptrac.yaml`'s Domain collector currently collapses all nine Domain modules (Admissions, Academic, Finance, HR, ...) into one layer** — meaning Domain-to-Domain dependencies (e.g. Admissions → Academic) are not mechanically checked at all today, despite the file's own comments describing that as forbidden ("never a sibling Domain module directly"). Recorded as a known gap in ADR-0026, not fixed there — narrowing the collector to per-module layers is a separate, larger decision affecting every existing Domain-to-Domain dependency in the codebase.

#### Sprint 4.4 — Enrollment lifecycle behavior

**Superseded scope note:** this section originally planned to build the `Enrollment` aggregate itself here. That schema shipped a sprint early, in Sprint 4.3, per an explicit architectural revision made before Sprint 4.3's implementation began (Enrollment built as a *complete* aggregate immediately, not a minimal one extended later). Sprint 4.4's real, final scope — settled across a dedicated Architecture Pass, not assumed — is the *behavior* that transitions an already-complete `Enrollment`: Promotion, Repetition, Withdrawal, Graduation. No schema changes.

**Goal:** the four state transitions named above, each a full Action with business rules, transaction boundaries, events, and tests.

**Scope — IN:** `PromoteEnrollmentAction`/`RepeatEnrollmentAction`/`WithdrawEnrollmentAction`/`GraduateEnrollmentAction` (all in Academic — see Layering Review below); `Enrollment::promote()`/`repeat()`/`withdraw()`/`graduate()`; `Student::withdraw()`/`graduate()` (the two methods Sprint 4.3's own docblock explicitly deferred here); `AcademicCatalogService::nextGradeLevel()`/`isFinalGradeLevel()`; six new events (`EnrollmentPromoted`/`EnrollmentRepeated`/`EnrollmentWithdrawn`/`EnrollmentGraduated`/`StudentWithdrawn`/`StudentGraduated`).

**Scope — OUT, explicitly (not silently dropped):**
- **Branch Transfer.** Deferred to an independent, later sprint — no historical data model exists yet for "branch changed mid-enrollment-period while preserving both facts," and mutating `branch_id` in place on the current `Enrollment` row would overwrite history (the same principle ADR-0014 already established for `MergeRequest`/`MergeReassignmentLog`: a historical record is never rewritten to track something forward). Needs either a `branch_transfer_records` sub-history table (mirroring `SuspensionRecord`) or a relaxed `unique(student_id, academic_year_id)` constraint — a schema decision correctly out of bounds for a behavior-only sprint. Worth designing alongside Phase 5's `Section` if a similar within-period transfer need exists there.
- `SectionAssignment`/`Section` (Phase 5, per Sprint 4.3's own deferral — unchanged).
- `SuspensionRecord`'s own suspend/reinstate Actions (schema shipped in Sprint 4.3; no Action writes to it yet, in either sprint — a real gap, not urgent, since nothing consumes it).
- Bulk/batch promotion (a job or admin-triggered mass operation) — this sprint is the single-`Enrollment` Action only, matching Sprint 4.3's own single-`Applicant` scope.

**Layering Review (added as a permanent first step for this sprint, per explicit instruction after Sprint 4.3's Finding 1 — a `deptrac` violation the implementation and its own verification pass both missed):** every new class's layer was declared and checked against `deptrac.yaml` *before* the rest of the design was approved, not after. The load-bearing decision: the four orchestrating Actions live in **Academic (Domain)** — each needs at least one Academic-owned fact (next `GradeLevel` in sequence, is the target `AcademicYear` open, is this the final grade) — while `Enrollment`/`Student`'s own new transition methods accept only plain IDs, never a `GradeLevel`/`AcademicYear` instance, exactly like `ConvertApplicantToStudentAction`'s existing pattern. Verified, not assumed: `deptrac analyse --no-cache` reports zero violations after implementation.

**Business rules resolved during implementation, not left ambiguous:**
- `ClosedAcademicYearGuard` is checked only against the *target* academic year for Promotion/Repetition, never against the closing `Enrollment`'s own (often already-closed) year — the guard exists to stop new commitments against a closed year, not to freeze a historical record's own terminal transition, which is an outcome of that year ending. Blocking it on that basis would prevent the exact end-of-year workflow the guard should enable. Withdrawal/Graduation check no academic-year guard at all, for the same reason.
- Withdrawal and Graduation are terminal (explicit user requirement): unlike `Applicant::convert()`'s defensive-backstop no-op (Sprint 4.3), all four transitions here throw `EnrollmentNotActiveException`/`StudentNotActiveException` on a non-active row — a promotion/repetition/withdrawal/graduation attempt against an already-decided `Enrollment` is a real caller mistake that must surface, not disappear.
- **Chain Invariant** (explicit user requirement, documented directly on `Enrollment`'s class docblock): every row has at most one `next_enrollment_id` and one `previous_enrollment_id`; no Action may produce a branching chain or a cycle. Holds structurally (`promote()`/`repeat()` are the only methods that ever set `next_enrollment_id`, both always create a brand-new row, and both refuse to run unless the row is still `STATUS_ACTIVE` — an active row can never already have a `next_enrollment_id`), proven by a dedicated multi-step chain test (promote twice, verify both directions, verify no other row claims either link).

**Definition of Done:** all four transitions provable end-to-end with their own failure cases; the Chain Invariant proven across a genuine multi-step sequence, not just one hop; a dual-connection MariaDB concurrency test proving the double-promotion guard, matching Sprint 4.3's own proof standard for `ConvertApplicantToStudentAction`; every new event proven deferred via `DB::afterCommit()` with a regression test forcing a rollback (Sprint 4.3's Finding 2 lesson applied from the start, not discovered again by review); `deptrac analyse --no-cache` run and green *before* implementation was considered complete, not just before commit.

**Testing checklist:** feature tests per behavior (happy path + failure cases: not-active, closed target year, no-next-grade, not-final-grade); the multi-step chain test; the concurrency test; two `afterCommit` regression tests.

**Git Milestone:** `v1.5-enrollment-lifecycle-behavior`

**Status: COMPLETE (2026-07-27).** Delivered exactly as scoped above, following a dedicated Architecture Pass (Layering Review first, per the standing instruction added after Sprint 4.3's own Finding 1) that went through two rounds of explicit review before implementation began: Branch Transfer was proposed, then correctly rejected and deferred (an in-place `branch_id` mutation would have overwritten history — see Scope-OUT above), and two additional invariants (terminal-state throwing, the Chain Invariant) were added to the spec before approval, not discovered afterward. One real implementation-time finding, caught by PHPStan during the sprint's own verification (not by a separate review): `$locked->student->graduate()`-style access through Eloquent's untyped `BelongsTo` relation left PHPStan unable to resolve `Student`'s own methods/properties (`method.notFound`/`property.notFound` — a real, avoidable error, distinct from the pre-existing `missingType.generics` debt already accepted elsewhere). Fixed by fetching `Student::findOrFail($locked->student_id)` explicitly in all four Actions, matching `ConvertApplicantToStudentAction`'s own established convention — not by annotating the relation's generics, which would be an inconsistent, one-relation-only fix ahead of the codebase-wide Larastan-ratcheting item already in the Technical Debt Register.

A subsequent independent read-only repository review found one further real gap the implementation's own verification pass missed: **`AcademicCatalogService::nextGradeLevel()`/`isFinalGradeLevel()` computed grade progression globally by `sequence_order` alone, never checking `BranchGradeLevel` at all** — despite `gradeLevelsForBranch()` already existing specifically to answer "which grades does this branch offer," and despite `Enrollment::promote()` keeping `branch_id` unchanged from the closing row. Concretely: Promotion could create an `Enrollment` at a grade the student's own branch doesn't teach (no FK catches this — the `GradeLevel` exists globally, just not at that branch); Graduation could be wrongly refused for a student who completed the last grade their branch offers, purely because a higher grade exists at a *different* branch. Not caught by the sprint's own 430 tests because none of them created a `BranchGradeLevel` row at all — the missing validation in the code and the missing setup in the fixtures cancelled out and produced green tests.

Fixed by renaming the methods to `nextGradeLevelForBranch()`/`isFinalGradeLevelForBranch()` (a `$branchId` parameter, joined through `BranchGradeLevel` the same way `gradeLevelsForBranch()` already does) — the rename itself was a deliberate part of the fix, not cosmetic, so a future caller can't reasonably assume either method is system-wide. `NoNextGradeLevelException` is deliberately still thrown for both "no higher grade exists at all" and "one exists, just not at this branch" — from the Action's own perspective the outcome is identical (promotion cannot proceed); a more specific exception is deferred until a real consumer needs to distinguish the two causes. `ClosedAcademicYearGuard`'s docblock (also flagged by the same review, low severity) was corrected to name Sprint 4.4's real consumers precisely. Two new regression tests added, each verified to fail against the pre-fix code by reasoning through the exact query change: promotion refused when the next grade exists globally but not at the branch; graduation succeeds at the branch's own final grade despite a higher grade existing elsewhere. Re-verified after the fix: 432/432 tests (18 new total across both review rounds), Pint clean, PHPStan zero new errors (187, unchanged), `deptrac analyse --no-cache` zero violations.
...
**Scope of this section:** Section, SectionAssignment, HomeroomAssignment (Sprint A/B), plus Subject Offering + subject-level Teacher Assignment (Sprint C, below). Timetables, Attendance, Grades/Grading Scale, Report Cards, Coordinator/Department-Head assignment, and Branch Transfer remain explicitly out of scope (Branch Transfer deliberately deferred until Sections shipped, per the Phase 5 Architecture Pass — see the Technical Debt Register).
...
**Status: COMPLETE AND CLOSED (2026-07-28).** Delivered exactly as scoped above. Verified: 455/455 tests (20 new), Pint clean, PHPStan zero new error *categories* (213, up from 187 — every additional finding confirmed to be the same pre-existing `missingType.generics`/`missingType.iterableValue` debt class already present on `GuardianStudent`'s own `temporalScopeAttributes()`, not a new kind of error), `deptrac analyse --no-cache` zero violations, migrations and rollback both proven. A subsequent **independent read-only repository review — fresh reads and fresh command re-runs, not a review of the implementation summary — found no correctness or architecture findings**: Layering/`deptrac` confirmed zero reverse dependency from People onto Academic; the Consistency Invariant confirmed enforced in `SectionAssignment`'s own `booted()` hook; `Enrollment.php` confirmed untouched by this sprint's commit; both Actions confirmed to open their transaction before `lockForUpdate()`; the concurrency test confirmed genuinely dual-connection; `capacity` confirmed to have zero hidden enforcement anywhere in the codebase; the "no events" deviation confirmed documented as intentional. Certified **APPROVED FOR FINAL CLOSURE** on this basis — the same evidentiary standard applied to Sprint 4.2/4.3/4.4, not a lighter pass because the architecture had already been reviewed twice before implementation.

### Sprint C — Subject Offering + Teacher Assignment (subject-level)

A dedicated Architecture Pass preceded this sprint (research: no `Subject`/`Term`/`SubjectOffering` code existed anywhere; a Draft→Published mechanism does not exist to reuse; no day/time-slot shape exists anywhere), followed by two review rounds. Two prior, separately-tracked ADR passes fed directly into this sprint's scope: **BUS-0032** ("Term Introduced as Master Data" — `docs/adr/business/0032-academic-term-introduced-as-master-data.md`, part of the still-uncommitted Business Blueprint governance arc) resolved `Term` as a real Master Data entity, child of `AcademicYear`, after an initial draft concluding "Term = AcademicYear" was explicitly reversed once a confirmed near-term commercial need for 2–3 terms/year surfaced; and the "Subject Offering + Timetables Architecture Pass" resolved `SubjectOffering`'s own shape.

**Scope — IN:**
- **`AcademicPeriodGuard`** (renamed/generalized from `ClosedAcademicYearGuard`, the same class, same file lineage) — `Term` became its second real consumer, matching Sprint A0's own "second real consumer forces a centralized fix" precedent (applied here to a guard, not `HasTemporalAssignment` itself) and BUS-0032's own recorded preference for a generalized guard over a duplicate `ClosedTermGuard`. `assertAcademicYearIsOpen()` is the pre-existing method, unchanged in behavior; `assertTermIsOpen()` is new, raising the new `ClosedTermException`.
- **`Term`** (Academic Master Data, per BUS-0032) — child of `AcademicYear`, mirrors its exact 3-state lifecycle (`upcoming`/`active`/`closed`), bilingual `name_en`/`name_ar`, `sequence_order` unique within `academic_year_id`. No `TermClosed` domain event — nothing consumes one yet (disclosed deviation from `AcademicYear`'s own precedent, not a silent omission).
- **`Subject`** — deliberately minimal Master Data (`code`/`name_en`/`name_ar`/`is_active` only), disclosed scope decision: BUS-0018's full formula richness (versioning, equivalency, prerequisites, department) is not built here because nothing consumes it yet.
- **`SubjectOffering`** — identity is the `(subject_id, section_id, term_id)` triple; `academic_year_id` stored directly (not derived via `term.academic_year_id`), per BUS-0032's required amendment. No `teacher_id` (Teacher Assignment is its own aggregate, mirroring `Section`/`HomeroomAssignment`'s precedent). **No status/`is_active` field at all** — explicit user decision: `SubjectOffering` is a "definition," not a "workflow"; it sits between two independently-lifecycled entities (`AcademicYear`, `Term`) and a boolean would create an unresolvable authority conflict between them. The three-way Consistency Invariant (`Section.academic_year_id == Term.academic_year_id == SubjectOffering.academic_year_id`) is enforced in `SubjectOffering::booted()`, raising the new `TermAcademicYearMismatchException` for either disagreeing side — the same `booted()::saving()` mechanism `SectionAssignment` already established, reused for a three-way check rather than the original two-way case BUS-0032 was scoped around. A 4th Consistency Invariant (`Section.grade_level` must support the `Subject`, via Curriculum) is **documented in the model's own docblock but deliberately not enforced** — Curriculum Path/Specification models don't exist yet (Addendum B1, "promotion not prediction").
- **`TeacherAssignment`** (subject-level) — mirrors `HomeroomAssignment` exactly, scoped to `subject_offering_id` instead of `section_id`; explicit single-cardinality assumption (one active teacher per `SubjectOffering`), flagged as an assumption for v1, not a permanent constraint on co-teaching.
- **`CreateSubjectOfferingAction`** — no row-locking (disclosed, deliberate deviation from `AssignSectionAction`/`AssignHomeroomTeacherAction`'s own locking discipline): `SubjectOffering` carries no `HasTemporalAssignment` overlap semantics, so its uniqueness is a plain DB unique index, already atomic under concurrent inserts without an application-level lock. Calls `AcademicCatalogService::assertTermIsOpen()` before creating, routing through the same "`AcademicCatalogService` is always consumed by an Action, never a model" convention every other closed-period check in this codebase already follows.
- **`AssignTeacherToSubjectOfferingAction`** — locks `SubjectOffering` (its own `temporalScopeAttributes()` anchor), matching every other Action's lock-the-natural-anchor-row discipline; calls `assertAcademicYearIsOpen()` against the locked offering's own year.

**Scope — OUT:** Room and Timetables — a hard, user-stated constraint, not a scheduling preference ("لا أسمح بإنشائه داخل Academic," i.e. Room may not be created inside Academic). A dedicated, independent ADR resolving Room's ownership (it will also be consumed by Exams, Labs, Booking, Events, and Smart Campus) must be produced and resolved before any Timetable code is written. No dual-connection concurrency test for `AssignTeacherToSubjectOfferingAction`'s lock — disclosed, deliberate: the underlying row-lock mechanism was already proven genuine twice (`HasTemporalAssignmentConcurrencyTest` at the trait level, `AssignSectionActionConcurrencyTest` at the Action level), and Sprint B itself set the precedent of not adding a third near-identical proof for `AssignHomeroomTeacherAction`, which this sprint's `TeacherAssignment` Action structurally mirrors.

**Domain ownership / Layering:** `Term`/`Subject`/`SubjectOffering`/`TeacherAssignment`/`AcademicPeriodGuard`/both new Actions all live in Academic (Domain) — no cross-Domain dependency introduced; `deptrac analyse --no-cache` confirms zero violations.

**Definition of Done:** a `SubjectOffering` whose `Section`/`Term`/own `academic_year_id` disagree is rejected by the three-way Consistency Invariant, proven by two separate tests (Section-side and Term-side mismatch); a duplicate `(subject_id, section_id, term_id)` offering is rejected at the DB level; one active `TeacherAssignment` per `SubjectOffering` is enforced via `HasTemporalAssignment`'s own overlap guard; `AcademicPeriodGuard::assertTermIsOpen()` rejects a closed Term and accepts an open/upcoming one; `SubjectOffering` is confirmed to carry no status/`is_active` attribute at all; `deptrac analyse --no-cache` reports zero violations.

**Testing checklist:** model-level tests for `Term` (uniqueness of `sequence_order` within year, per-year non-uniqueness across years, 3-state lifecycle, idempotent `close()`), `Subject` (uniqueness of `code`, active scope, deactivation), `SubjectOffering` (creation, both invariant-violation branches, duplicate-triple rejection, cross-Section non-uniqueness, no-status-column proof), `TeacherAssignment` (creation, overlap rejection, reassignment via `closeAssignment()`); Action-level tests for `CreateSubjectOfferingAction` (success, closed-Term rejection, invariant propagation) and `AssignTeacherToSubjectOfferingAction` (success, closed-Academic-Year rejection); `AcademicPeriodGuardTermTest` mirroring the existing `AcademicYearClosedGuardTest` for the Term-side method.

**Git Milestone:** `v1.8-subject-offering`

**Status: COMPLETE AND CLOSED (2026-07-28).** Verified: 479/479 tests (24 new), Pint clean, PHPStan 236 errors (up from 213 — all 23 net-new findings confirmed the same pre-existing `missingType.generics`/`missingType.iterableValue` debt class every other model in this codebase already carries, not a new kind of error), `deptrac analyse --no-cache` zero violations, migrations and rollback both proven. A subsequent **independent read-only repository review — fresh reads and fresh command re-runs, including a from-scratch Docker bootstrap, not a review of the implementation summary — found zero architecture, layering, invariant, schema, or concurrency findings**: `deptrac` confirmed zero violations and zero reverse dependency from People onto the new Academic entities; the three-way Consistency Invariant confirmed enforced in `SubjectOffering::booted()` via direct code inspection and two passing mismatch tests; `git diff` confirmed `Section.php`/`SectionAssignment.php`/`HomeroomAssignment.php`/`AcademicYear.php`/`HasTemporalAssignment.php` byte-identical to their pre-Sprint-C state; both Actions' transaction boundaries and `AssignTeacherToSubjectOfferingAction`'s `lockForUpdate()` confirmed correct; `CreateSubjectOfferingAction`'s deliberate no-lock decision confirmed sound (no `HasTemporalAssignment` overlap semantics exist to race, so the DB unique index alone is sufficient); BUS-0032 confirmed to match the implementation field-by-field. **One documentation-only finding**: the implementation's own verification report (and this document, and `CHANGELOG.md`, and the `a8ff7f2` commit message) had claimed "510/510 tests (31 new)" — an inflated figure produced by an overly broad `--filter="Term|Subject|TeacherAssignment|AcademicPeriodGuard"` match that incidentally pulled in 7 unrelated pre-existing tests via substring collisions (e.g. "termina**l**" matching "term"). Two independent fresh full-suite runs both returned 479 passed; a direct count of `it()` blocks across the 7 new test files totals 24; `455 (Sprint B's own documented baseline) + 24 = 479` reconciles exactly. Corrected here and in `CHANGELOG.md`, per the same policy applied to Sprint 4.4's own Finding 3 — a documentation-only correction, no code/test/migration change, no second full review required. Certified **APPROVED FOR FINAL CLOSURE**.
...
|---|---|---|---|
| **5 — Academic build-out (remaining epics)** | Timetables (blocked on a dedicated, independent ADR resolving Room's ownership — explicitly not decidable inside Academic, per Sprint C's own scope note); Attendance; Grades + Grading Scale (versioned per Addendum A4); Report Card generation + finalization snapshot; Branch Transfer (deliberately deferred until Sections shipped, per its own Phase 5 Architecture Pass note above) | Phase 5 Sprint C (Subject Offering + Teacher Assignment, now complete) | Building Grading Scale as a flat setting instead of a properly versioned entity — this was explicitly flagged as needing real versioning, not just a snapshot; designing Timetables (or any Room-consuming feature) before Room's ownership ADR is resolved |
...
| **Room ownership** | Undecided — explicitly NOT Academic (binding user instruction, Sprint C Architecture Pass: "لا أسمح بإنشائه داخل Academic") | Whenever Timetables (or any other Room-consuming feature: Exams, Labs, Booking, Events, Smart Campus) is next in the implementation queue | Needs its own dedicated, independent ADR (mirroring BUS-0032's own process) before any Timetable code line is written — Room is a cross-cutting facility resource, not a Timetable-specific concern, so deciding it inside Academic would repeat the exact category of module-placement mistake Sprint 4.3's own critical `deptrac` finding was |
...
| **`ApplicationSubmitted` dispatches inside its `DB::transaction()`, before commit** (found during Sprint 4.2's independent final review, 2026-07-27) | Whenever the first real listener is added (Sprint 4.3's conversion action is the named future consumer) | Inert today — no listener exists yet. If a future listener queries the `Applicant` row from a separate connection/process (e.g. a queued listener), it must use `afterCommit()`/`ShouldDispatchAfterCommit`, since the row isn't guaranteed durable until the transaction this dispatch sits inside actually commits. Not fixed now — adding `afterCommit()` handling with no consumer to justify it would itself be speculative. |
| **Sprint 4.2's transaction-atomicity fix necessarily lengthened the critical section under `NumberGeneratorService`'s row lock** (found during the same review, 2026-07-27) | Performance/load testing, Phase 4 (already scheduled below) | Fixing the transaction-atomicity bug (Guardian/PersonIdentityDocument writes must roll back with the Applicant) required moving `resolveGuardian()`, the Academic Year/Grade Level checks, and `resolveApplicantPerson()`'s duplicate-detection read inside the same transaction `NumberGeneratorService::next()`'s `lockForUpdate()` participates in — a longer lock hold than before the fix. Accepted as the correct trade-off (correctness over raw throughput); worth a real concurrency/load check once Phase 4's own performance-testing milestone (below) is reached, not before. |
| **Configuration Platform and Provider Registry's own List↔Detail navigation (Overview Grid → category/slot form) uses local `useState`, not the URL** (found during UI Sprint 1-A, 2026-07-30, when the same shortcut was about to be repeated for Academic) | Whenever either workspace is next touched for an unrelated reason — not a standalone task | Neither their category-rail/slot selection nor their two-level drill-down is deep-linkable or refresh-safe today, contradicting the binding rule newly recorded in `docs/ADMIN_DESIGN_SYSTEM.md` §8.5 ("workspace state that identifies a navigable resource lives in the URL"). UI Sprint 1-A's own Academic pattern (§28) is the first real consumer of the fix — a nested index+splat route under `router.tsx`'s per-workspace parent — but retrofitting these two already-shipped, frozen workspaces onto it is a real, separate migration, not bundled into Sprint 1-A's scope. |
| **`admin/src/platform/data-table/search-input.tsx`'s debounce duplicates `platform/search/search-bar.tsx`'s near-identical inline debounce logic** (found by UI Sprint 1-A's independent read-only review, 2026-07-30) | Whenever a third list-scoped debounce need arises | No shared `useDebouncedValue` hook exists in this codebase to reuse (disclosed in `search-input.tsx`'s own code comment, not a silently-copied shortcut) — extracting one with only two real consumers would have been speculative. |
| **`admin/src/platform/i18n/index.ts` statically imports `entity-workspace/locales/{en,ar}.json`** instead of using its own adjacent `registerWorkspaceTranslations()` extension point (found by UI Sprint 1-A's independent read-only review, 2026-07-30) | Whenever `i18n/index.ts` is next touched for an unrelated reason | Inconsistent with that file's own doc comment, which states a future workspace registers its own translations "without touching this file." Defensible as written — `entity-workspace` is platform infrastructure (like `shell`, already bundled the same way), not a workspace — but the inconsistency was never reconciled explicitly in the file's own comment, so it reads as an oversight rather than a deliberate exception. |
---
