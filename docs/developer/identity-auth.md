...
ADR-0008 also fixes what `Contact.is_primary` means: **preferred, scoped per contact type** (an independent preferred email and preferred phone, not one preferred contact overall). Any future module selecting a `Contact` to message must default to the preferred verified one of the type it needs, following the ADR's ordered fallback when none is designated — never picking arbitrarily among several verified candidates of the same type. Nothing in the current schema enforces one `is_primary` per person+type yet; that's named as a gap for whichever module first builds real delivery to close, not fixed in Sprint 2.2.
