...
**Future implementation considerations (recorded 2026-07-16, from a post-Phase-2 stress test against 19 hypothetical provider families — not scheduled, not defects, not a Phase 2 redesign).** The stress test confirmed the SDK generalizes unchanged to 15 of 19 families with genuinely different shapes. It also surfaced three natural evolution points, each real but narrow enough that none justifies touching the SDK, Registry, or Vault *now* — each should only be built when a real provider actually needs it, per the same B1 promotion-not-prediction discipline already governing `ApprovalRoutingResolver`'s independent copies:

1. **Credential Sensitivity Classification (public / secret / internal).** `ProviderCredentialVault` treats every field in a slot's `credentialFields` as uniformly secret. Some real vendors mix sensitivities in one credential set — Stripe's publishable key is meant to be frontend-exposed, its secret key and webhook-signing secret are not. Today's workaround needs no code change (route the non-secret field through Configuration Platform instead, the secret fields through the Vault) but nothing documents or enforces that split. If a third provider needs it, the smallest fix is a per-field `sensitive: bool` on `ProviderSlotDefinition` plus a `resolvePublic()` method on the Vault that only returns non-sensitive fields — not a schema change to `provider_credentials` itself.

2. **Active Provider Resolution (`resolveActive(capabilityContract)`).** The schema and `ProviderRegistry::sync()` already support multiple vendors registered under one `capability_contract` (never exercised — each Phase 2 proof provider was the only one in its category), but `ProviderManager::resolve()` only takes a literal `slot_key`. Swapping which vendor is "live" for a capability today means a code change everywhere the slot key is hardcoded, not a configuration change. The smallest fix is additive and `ProviderManager`-only: a `resolveActive(string $capabilityContract)` method, backed by a "which slot is active" selector — itself a strong candidate for a plain (non-secret) Configuration Platform key, reusing the existing mechanism rather than inventing a new one.

3. **Provider Lifecycle (stateless vs. connection-oriented).** Every Phase 2 proof provider (SMTP, OAuth, Push) is stateless-per-call, so `ProviderManager::resolve()`'s per-request container resolution was never stress-tested against a long-lived resource. A message-broker provider (RabbitMQ, Kafka, MQTT) resolved the same way would silently open a new connection per call rather than reusing a pool — a production correctness issue, not a registration-time one, so nothing in `ProviderRegistry::sync()`'s guards would catch it. The fix, when needed, is provider-implementation-only (the provider manages its own singleton connection internally); it does not require the Registry, Vault, or `ProviderManager` itself to gain a lifecycle concept, unless a future capability specifically needs the platform to manage that lifecycle on the provider's behalf.
