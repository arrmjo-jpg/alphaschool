...
**5. Credential audit handling, added by amendment (2026-07-14).** Neither this ADR nor ADR-0018 originally specified how Provider Credential changes are recorded for audit — a silent gap, surfaced by a dedicated pre-implementation critical review, that this codebase's own dominant convention (Activitylog's `LogsActivity` trait, applied to nearly every other model) would fill by default with a full before/after diff, writing plaintext secret values into a broadly-readable table. Provider Credentials instead use the cheap tier of Blueprint §6's already-established two-tier Audit Pattern — who/when only, never a value diff, in either direction. The Vault itself retains its own encrypted version history (a new credential write creates a new encrypted version; prior versions are retained, never overwritten, consistent with Blueprint §7's "never overwrite history" rule); Rollback (ADR-0021 Decision 10) for a credential reactivates a prior encrypted Vault version directly and never reads a credential value from the general Audit Engine.
