...
it('rejects a closed Academic Year and accepts an open one', function () {
    ...
    $guard = app(ClosedAcademicYearGuard::class);

    expect(fn () => $guard->assert($closedYear->id))
        ->toThrow(ClosedAcademicYearException::class);

    expect(fn () => $guard->assert($activeYear->id))->not->toThrow(Exception::class);
    expect(fn () => $guard->assert($upcomingYear->id))->not->toThrow(Exception::class);
});
